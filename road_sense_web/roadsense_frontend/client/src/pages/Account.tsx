...
export default function AccountPage() {
  ...
  const signOut = async () => {
    if (!window.confirm("Log out of RoadSense on this device?")) return;
  ...
  const avatarUrl = `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(account.name || account.email || "RoadSense")}&backgroundColor=61d0bc&textColor=0b1019&fontSize=42`;
  return <main className="account-stage"><header className="account-header"><Link href="/" className="auth-brand"><span><Crosshair size={18} /></span><strong>RoadSense.</strong></Link><button type="button" className="account-back" onClick={() => navigate("/")}><ArrowLeft size={15} />Dashboard</button></header><div className="account-stage__backdrop" aria-hidden="true"><span /><span /><span /></div><div className="account-stage__card"><AccountFlipCard initials={initials} name={account.name || "RoadSense user"} email={account.email || ""} avatarUrl={avatarUrl} incidentCount={account.incident_count} gmailConnected={account.gmail_connected} onInfo={() => navigate("/account/info")} onSignOut={() => void signOut()} /></div></main>;
}
